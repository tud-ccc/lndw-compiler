...
impl ShiftReplacement for Expr {
    fn replace_multiplications_with_bitshifts(self) -> Self {
        match self {
            ...
            Expr::UnaryOp(op, expr) => Expr::UnaryOp(op, Box::new(expr.replace_multiplications_with_bitshifts())),
            Expr::BinaryOp(lhs, o @ (Operator::Add | Operator::Sub | Operator::Shl | Operator::Shr), rhs) => {
                Expr::BinaryOp(
            ...
            Expr::BinaryOp(left, op @ (Operator::Mul | Operator::Div), right) => {
                ...
                if let &Expr::Num(lhs) = left.as_ref()
                    && (lhs & (lhs - 1)) == 0
                    && !matches!(op, Operator::Div)
                {
                    // mul only
                    Expr::BinaryOp(right, Operator::Shl, Box::new(Expr::Num(lhs.ilog2() as i32)))
